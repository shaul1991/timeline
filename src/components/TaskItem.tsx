...
export default function TaskItem({ task, onToggle, onDelete, onEdit, onSubtaskToggle }: TaskItemProps) {
        ...
        <div className="px-4 pb-4 pt-2 border-t border-gray-100 dark:border-gray-700">
          <div className="space-y-2">
            {task.subtasks!.map((subtask) => (
              <div key={subtask.id} className="flex items-center gap-2 pl-8">
                <input
                  type="checkbox"
                ...
                />
                <span className={`text-sm text-gray-700 dark:text-gray-300 ${subtask.completed ? 'line-through text-gray-500' : ''}`}>
                  {subtask.title}
                </span>
              </div>
            ))}
